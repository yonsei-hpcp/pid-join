...
#define BLOCK_LOG_2 10
#define BLOCK_SIZE_2 (1 << BLOCK_LOG_2)
#define ELEM_PER_BLOCK_2 (BLOCK_SIZE_2 >> 3)
    ...
    for (uint32_t i = 0; i < digit + 1; i++)
    {
        for (uint32_t j = 0; j < 8; j++)
        {
            ...
            for (uint32_t k = 0; k < size; k++)
                if ((arr[k].lvalue / factor) % 8 == j) arr_buff[idx++] = arr[k];
    ...
    for (uint32_t bucket_id = tasklet_id; bucket_id < histogram_bucket_num + 1; bucket_id += NR_TASKLETS)
    {
        ...
        else
        {
            ...
            for (uint32_t iter = 0; iter < (elem_num / ELEM_PER_BLOCK + 1); iter++)
            {
                ...
                mutex_lock(&(mutex_atomic[19]));
                radixSort(tuples_read_buff, (tuplePair_t *) sorted_buff2, sort_num);
                mutex_unlock(&(mutex_atomic[19]));
