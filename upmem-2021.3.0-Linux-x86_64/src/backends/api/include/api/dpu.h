 ...
 *
 * Memory operations between the Host and the DPUs are done with the functions dpu_broadcast_to(),
 * dpu_prepare_xfer() and dpu_push_xfer().
 */
 ...
 * @param set the dpu_set_t from which to extract the log (should be a single DPU)
...
dpu_error_t
do_rnc_job(
    rotate_n_stream_job_t* curr_job);
...
rotate_n_stream_job_t* rnc_queue_next_job(RNS_Job_Queue_t* job_queue);

void rnc_nqueue_add_job(RNS_Job_Queue_t* job_queue, const rotate_n_stream_job_t* new_job);
