...
__API_SYMBOL__  dpu_error_t
do_rnc_job(rotate_n_stream_job_t* job)
{
    ////////////////////////////////////
    dpu_error_t status;
    struct dpu_program_t *program;
...
/**
 * @brief dpu_set must be a single rank
 * 
 */
__API_SYMBOL__ dpu_error_t
unordered_scatter_rankwise(
    ...
    int num_threads)
{
    ...
    return unordered_data_transfer(dpu_set, data_ptr, symbol, symbol_offset, length, true, num_threads);
}

__API_SYMBOL__ dpu_error_t
unordered_gather_rankwise(
    struct dpu_set_t dpu_set,
    void* data_ptr,
    const char *symbol_name,
    uint32_t symbol_offset,
    ...
    int num_threads)
{
    ...
    return unordered_data_transfer(dpu_set, data_ptr, symbol, symbol_offset, length, false, num_threads);
}
...
{
    dpu_error_t status = DPU_OK;
    dpu_mem_max_addr_t mram_address = symbol.address + symbol_offset;
    ...
    switch (dpu_set.kind) 
    {
        case DPU_SET_RANKS: 
        {
            if (dpu_set.list.nr_ranks != 1)
            {
                printf("Error: Not supported number of rank %d. %s %d\n", dpu_set.list.nr_ranks, __FILE__, __LINE__);
                return DPU_ERR_INTERNAL;
            }
            else
            {
                // check symbol is valid
                CHECK_SYMBOL(symbol, symbol_offset, (length / 64));
                

                (mram_address) &= ~MRAM_MASK;
                // uint32_t nr_ranks = dpu_set.list.nr_ranks;
                struct dpu_rank_t **ranks = dpu_set.list.ranks;

                dpu_lock_rank(ranks[0]);
                status = dpu_copy_mrams_unordered_data_transfer(
                    ranks[0],
                    mram_address,
                    data_ptr,
                    length,
        ...
        break;
        default:
            printf("Error: Not supported dpu set time. %s %d\n", __FILE__, __LINE__);
            return DPU_ERR_INTERNAL;
    }
